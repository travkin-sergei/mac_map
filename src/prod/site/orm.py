...
def updateCountry(incoming_data):
    ...
    with session_sync() as session:
        old_object = session.query(Country).filter_by(hash_address=incoming_data.hash_address).first()
        if old_object is None:
            session.add(data_rec)
            session.commit()
...
def get_products_name():
    """
    Возвращает только название на английском языке
    """
    with session_sync() as session:
        result = session.query(Products.name).filter_by(is_plan=True, name_rus=None).distinct().yield_per(100)
        return result
...
@logger_fun
def set_products2(name_en, name_rus):
    """update name"""
    with session_sync() as session:
        odj = session.query(Products).filter_by(name=name_en)
        if odj:
            for i_odj in odj:
                i_odj.name_rus = name_rus
            session.commit()
